"""Panel SGTI alternativo en Streamlit (lee Data/parquet en vivo) — v3.

Ejecutar:
    .venv/bin/streamlit run streamlit_dashboard_v3.py

Copia de streamlit_dashboard_v2.py con cuatro features nuevas:
  1. Exportación CSV por pestaña (datos ya filtrados, UTF-8 con BOM).
  2. Ranking paramétrico top-N (slider global en sidebar).
  3. Heatmap semanal de actividad (oficios realizados + recibidos, Altair).
  4. Búsqueda global por subcadena (case-insensitive) en todos los campos.

Reutiliza el cargador/normalizador de scripts/build_dashboard.py y las métricas de
scripts/data_quality.py, así que las reglas de limpieza son idénticas a v2.
"""
import os
import sys
...
BUSQ = st.sidebar.text_input("Buscar…")
...
def filtro_global(df):
    term = BUSQ.strip().lower()
    if not term:
        return df
    joined = df.astype(str).agg(" ".join, axis=1).str.lower()
    return df[joined.str.contains(term, regex=False, na=False)]
...
def heatmap_semanal(a, b, title="Actividad semanal (oficios realizados + recibidos)", height=340):
    da = a[["fecha"]].copy()
    db = b[["fecha"]].copy()
    m = pd.concat([da, db], ignore_index=True)
    m = m[m["fecha"] != ""]
    m["dt"] = pd.to_datetime(m["fecha"], errors="coerce")
    m = m.dropna(subset=["dt"])
    if m.empty:
        return None
    m["semana"] = m["dt"].dt.isocalendar().week.astype(int)
    m["dia"] = m["dt"].dt.dayofweek.astype(int)
    h = m.groupby(["semana", "dia"]).size().reset_index(name="conteo")
    semanas = range(int(h["semana"].min()), int(h["semana"].max()) + 1)
    grid = pd.DataFrame([(s, d) for s in semanas for d in range(7)],
                        columns=["semana", "dia"])
    h = grid.merge(h, on=["semana", "dia"], how="left")
    h["conteo"] = h["conteo"].fillna(0).astype(int)
    h["dia_n"] = h["dia"].map(lambda d: DIAS_SEMANA[int(d)])
    low = "#2a3648" if DARK else "#dbeafe"
    return _theme(alt.Chart(h).mark_rect(stroke=_GRID, strokeWidth=0.5).encode(
        x=alt.X("dia_n:N", sort=DIAS_SEMANA, title="Día de la semana"),
        y=alt.Y("semana:O", sort="descending", title="Semana ISO"),
        color=alt.Color("conteo:Q",
                        scale=alt.Scale(domain=[0, max(1, int(h["conteo"].max()))],
                                        range=[low, C["blue"]]),
                        legend=alt.Legend(title="Registros")),
        tooltip=["dia_n", "semana", "conteo"],
    ).properties(title=title, height=height))
...
def download_button(df, filename, key, label="⬇ Descargar CSV"):
    st.download_button(
...
# ===== Resumen =====
with tabs[0]:
    OR, RR, NT, CU, EN, RE = (filtro_global(filtro_fecha(DATA[k])) for k in
                              ("or", "rr", "nt", "cu", "en", "re"))
    ...
    st.altair_chart(monthly_grouped(OR, RR, "Realizados", "Recibidos", "Oficios por mes"),
                    width="stretch")
    st.altair_chart(monthly(filtro_global(filtro_fecha(DATA["nt"])), "Notas informativas por mes",
                            color=C["purple"], kind="line"), width="stretch")

# ===== Oficios realizados =====
with tabs[1]:
    df = filtro_global(filtro_fecha(DATA["or"]))
    f1, f2 = st.columns(2)
    with f1:
    ...
    with cb:
        download_button(df, "oficios_realizados.csv", "dl_or")
    c1, c2 = st.columns(2)
    with c1:
        st.altair_chart(donut(df, "estado", "Por estado"), width="stretch")
    with c2:
        st.altair_chart(monthly(df, "Por mes"), width="stretch")
    st.altair_chart(hbar_top(df, "area", f"Por área (top {TOP_N})", n=TOP_N), width="stretch")
    tabla(df, ["fecha", "numero", "area", "estado", "asunto"])

# ===== Oficios recibidos =====
with tabs[2]:
    df = filtro_global(filtro_fecha(DATA["rr"]))
    f1, f2 = st.columns(2)
    with f1:
    ...
    with cb:
        download_button(df, "oficios_recibidos.csv", "dl_rr")
    c1, c2 = st.columns(2)
    with c1:
        st.altair_chart(donut(df, "estado", "Por estado"), width="stretch")
    with c2:
        st.altair_chart(monthly(df, "Por mes", color=C["teal"]), width="stretch")
    st.altair_chart(hbar_top(df, "area", f"Por área (top {TOP_N})", n=TOP_N, color=C["teal"]),
                    width="stretch")
    tabla(df, ["fecha", "numero", "remitente", "area", "estado", "asunto"])

# ===== Notas informativas =====
with tabs[3]:
    df = filtro_global(filtro_fecha(DATA["nt"]))
    df = filtro_multiselect(df, "area", "Área (TI)", "nt")
    cc, cb = st.columns([4, 1])
    with cc:
        st.caption(f"Mostrando {len(df)} notas informativas.")
    with cb:
        download_button(df, "notas_informativas.csv", "dl_nt")
    st.altair_chart(monthly(df, "Notas por mes", color=C["purple"], kind="line"),
                    width="stretch")
    st.altair_chart(hbar_top(df, "area", f"Por área (top {TOP_N})", n=TOP_N, color=C["purple"]),
                    width="stretch")
    tabla(df, ["fecha", "area", "autor", "asunto"])

# ===== Cuentas de usuario =====
with tabs[4]:
    df = filtro_global(filtro_fecha(DATA["cu"]))
    f1, f2 = st.columns(2)
    with f1:
    ...
    with cb:
        download_button(df, "cuentas_usuario.csv", "dl_cu")
    c1, c2 = st.columns(2)
    with c1:
    ...
    st.altair_chart(hbar_top(df, "area", f"Por coordinación (top {TOP_N})", n=TOP_N,
                             color=C["teal"]), width="stretch")
    tabla(df, ["fecha", "persona", "username", "area", "estado"])

# ===== Suministros =====
with tabs[5]:
    en = filtro_global(filtro_fecha(DATA["en"]))
    ing = filtro_global(filtro_fecha(DATA["ing"]))
    f1, f2 = st.columns(2)
    with f1:
    ...
    cc, cb, cb2 = st.columns([3, 1, 1])
    with cc:
        st.caption(f"Mostrando {len(en)} entregas y {len(ing)} ingresos.")
    with cb:
        download_button(en, "entregas_suministro.csv", "dl_en")
    with cb2:
        download_button(ing, "ingresos_suministro.csv", "dl_ing")
    c1, c2 = st.columns(2)
    with c1:
    ...
    st.altair_chart(hbar_top(en, "area", f"Entregas por área (top {TOP_N})", n=TOP_N,
                             color=C["teal"]), width="stretch")
...
# ===== Operaciones =====
with tabs[6]:
    re, li, va, au = (filtro_global(filtro_fecha(DATA[k])) for k in ("re", "li", "va", "au"))
    c1, c2 = st.columns(2)
    with c1:
    ...
    c3, c4 = st.columns(2)
    with c3:
        st.altair_chart(hbar_top(li, "ap", f"Licencias por aplicativo (top {TOP_N})", n=TOP_N,
                                 color=C["pink"]), width="stretch")
    with c4:
    ...
    st.altair_chart(hbar_top(au, "modulo_lbl", f"Actividad por módulo (top {TOP_N})", n=TOP_N,
                             color=C["gray"]), width="stretch")
    st.subheader("Detalle de reubicaciones")
    download_button(re, "reubicaciones.csv", "dl_re")
    tabla(re, ["fecha", "tipo", "equipo", "origen", "area"])
    st.subheader("Detalle de licencias")
    download_button(li, "licencias.csv", "dl_li")
